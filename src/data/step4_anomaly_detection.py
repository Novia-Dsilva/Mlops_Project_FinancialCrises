...
>>>>>>> 8d1ecdb96c99c112cf86a9fd5fc949c4961a8f42:src/data/step4_anomaly_detection.py
        ...
        logger.info(
            f"   Will create flags for {len(cols_to_flag)} key columns...")
...
>>>>>>> 8d1ecdb96c99c112cf86a9fd5fc949c4961a8f42:src/data/step4_anomaly_detection.py
        ...
        if outlier_summary:
            ...
            logger.info(
                f"   Found outliers in {len(outlier_summary)} column-group combinations:")
            print(outlier_df.head(10).to_string(index=False))

            flags_created = [
                col for col in df_flagged.columns if col.endswith('_Outlier_Flag')]
            logger.info(
            ...
            self.anomaly_report['total_count'] += sum(
                item['count'] for item in outlier_summary)
        else:
...
>>>>>>> 8d1ecdb96c99c112cf86a9fd5fc949c4961a8f42:src/data/step4_anomaly_detection.py
        ...
        logger.info(
            f"   Checking {len(available_cols)} columns for sudden jumps...")
        ...
        for col in available_cols:
            ...
            if group_col and group_col in df.columns:
                for company in df[group_col].unique():
                    company_df = df[df[group_col] ==
                                    company].sort_values('Date').copy()
...
<<<<<<< HEAD:src/data/step6_anomaly_detection.py
                ...
                if jump_count > 0:
                    ...
                    jump_years = [
                        int(year) for year in df_sorted.loc[jump_mask, 'Date'].dt.year.unique()]
...
>>>>>>> 8d1ecdb96c99c112cf86a9fd5fc949c4961a8f42:src/data/step4_anomaly_detection.py
        if temporal_anomalies:
            ...
            for anomaly in temporal_anomalies[:10]:
                severity_icon = "📊" if anomaly.get(
                    'is_crisis', False) else "⚠️"
            ...
            flags_created = [
                col for col in df_flagged.columns if col.endswith('_Jump_Flag')]
            logger.info(
...
════════════════════════════════════════════════════════════════
    ...
    def _save_report(self):
        ...
        json_path = output_dir / \
            f"anomaly_report_{self.dataset_name}_{timestamp}.json"
...
>>>>>>> 8d1ecdb96c99c112cf86a9fd5fc949c4961a8f42:src/data/step4_anomaly_detection.py
        ...
        if self.anomaly_report.get('statistical_outliers'):
            csv_path = output_dir / \
                f"outlier_summary_{self.dataset_name}_{timestamp}.csv"

            try:
                outlier_df = pd.DataFrame(
                    self.anomaly_report['statistical_outliers'])
