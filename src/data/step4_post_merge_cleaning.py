...
class PostMergeDataCleaner:
    ...
    def compute_statistics(self, df: pd.DataFrame, name: str) -> Dict:
        ...
        if not numeric_df.empty:
            ...
            non_negative_cols = ['VIX', 'Volume',
                                 'CPI', 'Total_Assets', 'Revenue']
            negative_count = 0
    ...
    def remove_duplicate_columns(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        suffixes = ['_x', '_y', '_fred', '_market',
                    '_macro', '_company', '_dup', '_fin']
            ...
            for suffix in suffixes:
                if col.endswith(suffix):
                    ...
                    if base_col in df.columns:
                        ...
                        logger.info(
                            f"   Found duplicate: '{col}' (keeping '{base_col}')")
                    else:
        ...
        if cols_to_rename:
            ...
            logger.info(
                f"   ✓ Renamed {len(cols_to_rename)} orphaned suffixed columns")
        ...
        logger.info(
            f"   Columns: {original_cols} → {final_cols} (removed {original_cols - final_cols})")
    ...
    def handle_inf_values(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if inf_before > 0:
            logger.info(
                f"   Found {inf_before:,} inf values across {len(numeric_cols)} numeric columns")
            ...
            # Show top offenders
            inf_cols_sorted = sorted(
                inf_cols, key=lambda x: x[1], reverse=True)
    ...
    def cap_extreme_outliers(self, df: pd.DataFrame,
            ...
            for col in numeric_cols:
                ...
                for group_name in df[group_col].unique():
                    ...
                    if group_data.notna().sum() > 10:  # Need enough data
                        ...
                        n_capped = ((group_data < lower) |
                                    (group_data > upper)).sum()
                        col_capped += n_capped

                        # Cap values
                        df.loc[group_mask, col] = group_data.clip(
        ...
        if capped_count > 0:
            logger.info(
                f"   ✓ Capped {capped_count:,} extreme values across {len(numeric_cols)} columns")

            # Show top capped columns
            capped_sorted = sorted(
                capped_details, key=lambda x: x[1], reverse=True)
    ...
    def handle_missing_values_post_merge(self, df: pd.DataFrame,
        ...
        logger.info(
            f"   Total missing values: {original_missing:,} ({original_missing_pct:.2f}%)")
        ...
        if len(high_missing) > 0:
            ...
            logger.info(
                f"   Note: Keeping these columns but they may be dropped later")
        ...
        logger.info(
            f"   Remaining: {final_missing:,} ({final_missing_pct:.2f}%)")
    ...
    def remove_constant_columns(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if low_variance_cols:
            df.drop(columns=low_variance_cols, inplace=True)
            logger.info(
                f"   ✓ Removed {len(low_variance_cols)} low-variance columns:")
            for col in low_variance_cols[:5]:
                logger.info(f"      - {col}")
        ...
        for col in ratio_cols:
            if col in df.columns:
                ...
                if extreme_high > 0 or extreme_low > 0:
                    ...
                    fixes.append(
                        f"{col}: capped {extreme_high + extreme_low} extreme values to [-1000, 1000]")
                ...
                always_positive = ['current_ratio',
                                   'debt_to_assets', 'volume_vs']
                ...
                if any(keyword in col.lower() for keyword in always_positive):
                    ...
                    if neg_count > 0:
                        df[col] = df[col].abs()
                        fixes.append(
                            f"{col}: made {neg_count} negative values positive")
        ...
        if 'Date' in df.columns and 'Company' in df.columns:
            ...
            if duplicates_before > 0:
                df = df.drop_duplicates(
                    subset=['Date', 'Company'], keep='first')
                duplicates_removed = original_rows - len(df)
                logger.info(
                    f"   ✓ Removed {duplicates_removed:,} duplicate (Date, Company) pairs")
            else:
            ...
            if duplicates_before > 0:
                ...
                duplicates_removed = original_rows - len(df)
                logger.info(
                    f"   ✓ Removed {duplicates_removed:,} duplicate dates")
            else:
    ...
    def clean_macro_features(self, df: pd.DataFrame) -> Tuple[pd.DataFrame, Dict, Dict]:
        ...
        logger.info(
            f"  Missing: {before_stats['total_missing']:,} ({before_stats['missing_pct']:.2f}%)")
        logger.info(f"  Inf values: {before_stats.get('inf_values', 0):,}")
        logger.info(
            f"  Invalid negatives: {before_stats.get('invalid_negatives', 0):,}")
        ...
        logger.info(f"  Shape: {df.shape}")
        logger.info(
            f"  Missing: {after_stats['total_missing']:,} ({after_stats['missing_pct']:.2f}%)")
        logger.info(f"  Inf values: {after_stats.get('inf_values', 0):,}")
        logger.info(
            f"  Invalid negatives: {after_stats.get('invalid_negatives', 0):,}")
    ...
    def clean_merged_features(self, df: pd.DataFrame) -> Tuple[pd.DataFrame, Dict, Dict]:
        ...
        df = self.remove_duplicate_columns(df)
        df = self.handle_inf_values(df)
        df = self.cap_extreme_outliers(
            df, group_col='Company')  # Cap per company
        df = self.handle_missing_values_post_merge(
            df, group_col='Company')  # Fill per company
        df = self.validate_data_types(df)
        df = self.remove_constant_columns(df)
        df = self.fix_invalid_ratios(df)
        df = self.remove_duplicates(df)
    ...
    def run_post_merge_cleaning(self):
        ...
        if macro_path.exists():
            ...
            df_macro_clean, before_macro, after_macro = self.clean_macro_features(
                df_macro)
            ...
            all_stats['macro_features'] = {
                'before': before_macro, 'after': after_macro}
        else:
            logger.warning(
        ...
        if merged_path.exists():
            ...
            df_merged_clean, before_merged, after_merged = self.clean_merged_features(
                df_merged)
            ...
            all_stats['merged_features'] = {
                'before': before_merged, 'after': after_merged}
        else:
            logger.warning(
        ...
        logger.info("\n➡️  Next Steps:")
        logger.info(
            "   1. (Optional) Validate: python src/validation/validate_checkpoint_4_clean_merged.py")
