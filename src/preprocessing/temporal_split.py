...
def create_date_based_splits(
    ...
    if not target_cols:
        raise ValueError(
            "No target columns found! Run create_targets.py first!")
    ...
    if only_train:
        print(
            f"   ⚠️  {len(only_train)} companies ONLY in train: {list(only_train)[:3]}...")
    if only_val:
        print(
            f"   ⚠️  {len(only_val)} companies ONLY in val: {list(only_val)[:3]}...")
    if only_test:
        print(
            f"   ⚠️  {len(only_test)} companies ONLY in test: {list(only_test)[:3]}...")
    ...
    print(
        f"      Size: {Path(train_file).stat().st_size / (1024*1024):.1f} MB")
    ...
    print(
        f"   Train: {len(train_df):,} rows ({train_pct:.1f}%) | {train_start.date()} to {train_end_actual.date()}")
    print(
        f"   Val:   {len(val_df):,} rows ({val_pct:.1f}%) | {val_start.date()} to {val_end_actual.date()}")
    print(
        f"   Test:  {len(test_df):,} rows ({test_pct:.1f}%) | {test_start.date()} to {test_end_actual.date()}")
