...
Handles outliers AFTER temporal split AND supports optional percentage-change
transformation for all numeric columns (features + targets).
...
# ============================================================
# CONFIG FLAGS (YOU CAN EDIT THESE)
# ============================================================
APPLY_PERCENT_CHANGE = True         # Apply % change to all numeric feature columns
# Convert target_* columns to % change (use only if desired)
APPLY_PCT_TO_TARGETS = False
# Keep first row after pct_change instead of dropping
KEEP_FIRST_VALUE = True
SKIP_OUTLIER_FOR_PERCENT_COLS = True
# ============================================================
...
# ------------------------------------------------------------
def convert_to_percent_change(df: pd.DataFrame, target_cols: list) -> pd.DataFrame:
    """
    Convert numeric columns to percentage change.
    Does NOT apply to targets unless APPLY_PCT_TO_TARGETS=True.
    """
    df = df.copy()
    ...
    print("\n🔄 Applying % change to numeric feature columns...")

    for col in feature_cols:
        df[col] = df[col].pct_change()

    if APPLY_PCT_TO_TARGETS:
        print("⚠️ Converting TARGET columns to % change also!")
        for col in target_cols:
            df[col] = df[col].pct_change()

    if KEEP_FIRST_VALUE:
        df.fillna(0, inplace=True)  # First diff becomes 0
    else:
        df.dropna(inplace=True)

    print(f"   Converted {len(feature_cols)} feature columns.")
    return df
...
# OUTLIER FUNCTIONS
# ------------------------------------------------------------
def calculate_winsorization_thresholds(train_df, target_col, lower_percentile, upper_percentile):
    valid_data = train_df[target_col].dropna()
    if len(valid_data) == 0:
        raise ValueError(
            f"No valid data in {target_col} for threshold calculation")

    return (
        float(valid_data.quantile(lower_percentile / 100.0)),
        float(valid_data.quantile(upper_percentile / 100.0)),
    )


def apply_winsorization(df, target_col, lower_cap, upper_cap, set_name=""):
    df_clean = df.copy()
    ...
    df_clean[target_col] = df_clean[target_col].clip(
    ...
    return df_clean, {
        ...
        "lower_count": int(lower_count),
        "upper_count": int(upper_count),
        "pct_capped": float((lower_count + upper_count) / total * 100),
        "lower_cap": float(lower_cap),
        "upper_cap": float(upper_cap),
    }
...
def handle_outliers_after_split(
    ...
    print("=" * 80)
    print("🔥 OUTLIER HANDLING + PERCENT CHANGE VERSION")
    print("=" * 80)
    ...
    # Load splits
    train_path = Path(splits_dir) / "train_data.csv"
    val_path = Path(splits_dir) / "val_data.csv"
    test_path = Path(splits_dir) / "test_data.csv"
    ...
    print(
        f"\nLoaded: Train {train_df.shape}, Val {val_df.shape}, Test {test_df.shape}")
    ...
    print(f"\nTargets found: {target_cols}")

    # ----------------------------------------
    # 1. APPLY % CHANGE BEFORE OUTLIERS
    # ----------------------------------------
    if APPLY_PERCENT_CHANGE:
        print("\n==============================")
        print("🔄 Converting to % change...")
        print("==============================")

        train_df = convert_to_percent_change(train_df, target_cols)
        val_df = convert_to_percent_change(val_df, target_cols)
        test_df = convert_to_percent_change(test_df, target_cols)

    # ----------------------------------------
    # 2. APPLY OUTLIERS (ONLY FOR TARGETS)
    # ----------------------------------------
    print("\n==============================")
    print("📉 Applying outlier winsorization...")
    print("==============================")

    if target_configs is None:
        # Default thresholds
        target_configs = {
            col: {"lower_percentile": 1.0,
                  "upper_percentile": 99.0, "description": col}
            for col in target_cols
        }
    ...
    for target_col in target_cols:
        ...
        # Skip outlier handling if percentage column
        if SKIP_OUTLIER_FOR_PERCENT_COLS:
            print(f"⏭️ Skipping outlier capping for % column: {target_col}")
            continue

        cfg = target_configs[target_col]

        # Calculate thresholds from TRAIN only
        lower_cap, upper_cap = calculate_winsorization_thresholds(
            train_df, target_col, cfg["lower_percentile"], cfg["upper_percentile"]
        )
        all_thresholds[target_col] = {
            "lower_cap": lower_cap,
            "upper_cap": upper_cap,
        ...
        # Apply to all
        train_df, _ = apply_winsorization(
            train_df, target_col, lower_cap, upper_cap, "Train")
        val_df, _ = apply_winsorization(
            val_df, target_col, lower_cap, upper_cap, "Val")
        test_df, _ = apply_winsorization(
            test_df, target_col, lower_cap, upper_cap, "Test")
    ...
    train_df.to_csv(Path(output_dir) / "train_data.csv", index=False)
    val_df.to_csv(Path(output_dir) / "val_data.csv", index=False)
    test_df.to_csv(Path(output_dir) / "test_data.csv", index=False)

    with open(Path(output_dir) / "outlier_thresholds.json", "w") as f:
        json.dump(all_thresholds, f, indent=2)

    print("\n✅ Done! Saved updated datasets.")
    return train_df, val_df, test_df, all_thresholds


if __name__ == "__main__":
    handle_outliers_after_split(
        splits_dir="data/splits",
        output_dir="data/splits",
        target_configs=None,
    )
