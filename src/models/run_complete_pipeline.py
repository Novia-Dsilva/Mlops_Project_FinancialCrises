...
def main():
    ...
    if train_models.lower() in ['yes', 'y']:
        ...
        if not run_script('src/models/Dense_VAE_optimized_mlflow_updated.py.py'):
        ...
        if train_ensemble.lower() in ['yes', 'y']:
            ...
            if not run_script('src/models/Ensemble_VAE_updated.py.py'):
...
def generate_deployment_summary(best_model):
    ...
    with open(report_path, 'w', encoding='utf-8') as f:  # Add encoding='utf-8'
        ...
        if selection_data:
            ...
            f.write(
                f"   KS Pass Rate: {metrics.get('ks_pass_rate', 'N/A'):.1f}%\n")
            f.write(
                f"   Correlation MAE: {metrics.get('correlation_mae', 'N/A'):.4f}\n")
        ...
        if validation_data:
            ...
            f.write(
                f"   KS Pass Rate: {val_metrics.get('ks_pass_rate', 'N/A'):.1f}%\n")
            f.write(
                f"   Wasserstein Distance: {val_metrics.get('wasserstein_mean', 'N/A'):.2f}\n")
