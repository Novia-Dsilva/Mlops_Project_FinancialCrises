...
Creates % CHANGE (growth rate) target variables for next-quarter prediction.
Targets generated:
1. target_revenue_pct
2. target_eps_pct
3. target_debt_equity_pct
4. target_profit_margin_pct
5. target_stock_return_pct
...
def pct_change(curr, nxt):
    """Safe percentage change calculation."""
    return (nxt - curr) / (np.abs(curr) + 1e-9) * 100


def create_targets(input_file: str, output_file: str):

    print("="*90)
    print("📈 CREATING NEXT-QUARTER % CHANGE (GROWTH) TARGETS")
    print("="*90)
    ...
    # ======================================================
    # RAW TARGETS (next-quarter values)
    # ======================================================
    ...
    raw_targets = {
        ...
        "target_profit_margin": "net_margin_q",
        "target_stock_return": "stock_q_return"
    }

    print("\n➡️ Creating raw shifted targets (intermediate)...")

    for tgt, src in raw_targets.items():
        if src not in df.columns:
            print(f"  ❌ Missing column: {src} → cannot create {tgt}")
            df[tgt] = np.nan
            continue

        df[tgt] = df.groupby("Company")[src].shift(-1)
        print(f"  ✓ {tgt} created")

    # ======================================================
    # CREATE PERCENTAGE CHANGE TARGETS
    # ======================================================

    print("\n➡️ Converting all targets into % CHANGE...")

    pct_targets = {}

    for tgt, src in raw_targets.items():
        pct_col = tgt + "_pct"  # new column
        base_col = src  # current quarter actual value

        if src not in df.columns:
            print(f"  ❌ Cannot compute % change for {src}")
            df[pct_col] = np.nan
            continue

        df[pct_col] = df.groupby("Company").apply(
            lambda g: pct_change(g[src], g[tgt])
        ).reset_index(level=0, drop=True)

        # Replace impossible values
        df[pct_col] = df[pct_col].replace([np.inf, -np.inf], np.nan)

        # Optional: Fill the null last quarter with 0
        df[pct_col] = df[pct_col].fillna(0)

        print(f"  ✓ {pct_col} created")

        pct_targets[pct_col] = pct_col

    # ======================================================
    # REMOVE RAW TARGETS
    # ======================================================

    print("\n🧹 Removing raw target values (keeping only % change)...")

    for tgt in raw_targets.keys():
    ...
    print("\n" + "="*90)
    print("📊 SUMMARY OF % CHANGE TARGETS")
    print("="*90)

    for col in pct_targets.values():
        print(f"\n{col}:")
    ...
    print("\n" + "="*90)
    print("💾 Saved file with % change targets")
    print("="*90)
    print(f"Output: {output_file}")
    print(f"Rows: {len(df):,}, Columns: {len(df.columns)}")
    print("Targets created:")
    for col in pct_targets.keys():
        print(f"  • {col}")


if __name__ == "__main__":
    input_path = "data/processed/features_engineered.csv"
    output_path = "data/features/quarterly_data_pct_targets.csv"

    create_targets(input_path, output_path)

    print("\n🚀 Ready for temporal split and TCN training!")
